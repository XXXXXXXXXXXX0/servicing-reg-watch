...
JURIS_RE = re.compile(r"^(federal|multi-state|US-[A-Z]{2})$")
...
def validate_rows(rows: list[dict]) -> list[str]:
    ...
    for r in rows:
        ...
        unresearched = r.get("status") == "unresearched"
        for k in REQUIRED:
            if unresearched and k in UNRESEARCHED_NULL:
                if k not in r:
                    errors.append(f"{where}: key {k} must be present (null for unresearched)")
                elif r[k] is not None:
                    errors.append(f"{where}: unresearched rows must have {k}: null")
